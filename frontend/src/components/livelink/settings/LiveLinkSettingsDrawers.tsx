...
import PresetDrawer, { PRESET_TAB_PREFIX } from './PresetDrawer'
 ...
 * sources and the broker their own, a preset's sensors (Mopeka) the preset
 * drawer, every other MQTT device the MQTT device drawer, and any other
 * registered kind the device-list fallback (spec G2).
 *
...
type DrawerKind = 'broker' | 'wican' | 'torque' | 'preset' | 'mqtt' | 'devices'
...
function drawerFor(tab: IntegrationTab): DrawerKind {
  ...
  // Before the kind: a preset's tab is generic_mqtt too, and holds every
  // sensor made from it rather than one device.
  if (tab.id.startsWith(PRESET_TAB_PREFIX)) return 'preset'
  // Any other MQTT device, mapped by hand or left by a retired preset.
...
export default function LiveLinkSettingsDrawers({ target, onClose, onChanged }: Props): ReactElement {
  ...
  return (
    <>
      ...
      <PresetDrawer open={openKind === 'preset'} tab={lastTab} onClose={onClose} onChanged={onChanged} />
